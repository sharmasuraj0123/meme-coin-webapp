...
export default function Page() {
    ...
    useEffect(() => {
        if (approvalSuccess === true && walletAddress !== undefined && stakedValue > 0) {
            waitForTransaction({ hash: approvalData?.hash as `0x${string}`}).then(_res => {
                refetchAllowance().then(res => {
                    setContractApprovalAmount(res.data as bigint);
                });
                toast.success("Contract is approved");
                supplyWrite({
                    args: [stakedValue * Number(process.env.NEXT_PUBLIC_TOKEN_DECIMAL ?? 0)],
                    from: walletAddress,
    ...
    const stakePion = () => {
        if (isEmpty(walletAddress) || walletAddress === undefined) {
            ...
            const stakedValueBigInt = BigInt(stakedValue * 
            ...
            if (BigInt(contractApprovalAmount) >= stakedValueBigInt) {
                ...
                approvalWrite({
                    args: [process.env.NEXT_PUBLIC_STAKING_CONTRACT_ADDRESS,
                        MAX_UINT256],
                    from: walletAddress,
                } as UseContractWriteConfig);
            }
